...
format:
  revealjs:
    ...
    footer: "[Week 3 - English](slides.html) | **Hafta 3 - Türkçe**"
...
1.  tidyverse ve janitor kütüphanelerini kullan; böylece bu kütüphanelerin veri görselleştirme için sunduğu fonksiyonlarını kullanabiliriz
2.  Verim bu yolda (yol = verinin hangi adreste olduğu) bulunan bir csv dosyası, onu oku ve şunu yap ( %\>% )
3.  `tibble` olarak göster: tibble verimizi görmenin güzel bir yolu. `-> earnings` ile "bu veri setini belleğe (RAM) kaydet, istediğimde `earnings` yazarak kullanayım" diyoruz.
...
::: fragment
[Sayısal değişkenler]{.red-text} hakkında değişken adlarını ve bilgileri görmek için `summary()` fonksiyonunu kullanıyoruz.
:::
...
## Gelir Nasıl Dağılıyor? {.small-slide}
...
4.  Yoğunluk grafiği ekle: `geom_density()`
:::
